...
def qust_7():
    ...
    return data_df
def qust_8():
    mycursor.execute("SELECT DISTINCT channel_data.channel_name FROM channel_data JOIN video_data ON channel_data.channel_name = video_data.channel_name WHERE EXTRACT(YEAR FROM video_data.published_date) = 2022;")
    data= mycursor.fetchall()
    data_df = pd.DataFrame(data, columns=['CHANNEL_NAME']).reset_index(drop=True)
    data_df.index += 1
    return data_df
def qust_9():
